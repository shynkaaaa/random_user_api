...
from src.repositories.user_repository import create_user, get_user_by_email, get_all_registered_users, get_user_by_id, update_user_repo, delete_user_repo
...
def delete_user_by_id(id: int, db: Session):
    if not get_user_by_id(id, db):
        raise HTTPException(status_code=404, detail="No such user to delete is found.")
    
    return delete_user_repo(id, db)
